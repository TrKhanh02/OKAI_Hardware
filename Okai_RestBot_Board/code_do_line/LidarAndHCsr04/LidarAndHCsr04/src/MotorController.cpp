...
void MotorController::init() {
    motorSerial->begin(115200, SERIAL_8N1, Config::MOTOR_RX_PIN, Config::MOTOR_TX_PIN);
...
void MotorController::sendCommand(int angelA, int dutyAngelA, int angelB, int dutyAngelB, 
    ...
    if (isSameCommand && (millis() - lastSentTime < 50)) {
        return; 
    }
    char buf[128];
...
void MotorController::stop() {
    lastCommand.m1_duty = -1; 
    lastCommand.m2_duty = -1;
